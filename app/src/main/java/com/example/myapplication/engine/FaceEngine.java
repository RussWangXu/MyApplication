...
public class FaceEngine {
    ...
    public static void init(){
        new Thread(new Runnable() {
            @Override
            public void run() {///storage/emulated/0/seetaface/SeetaFaceDetector2.0.ats
                FACEDETECTOR = new FaceDetector2(Environment.getExternalStorageDirectory().getPath()+"/seetaface"+File.separator+"SeetaFaceDetector2.0.ats");
                POINTDETECTOR = new PointDetector2(Environment.getExternalStorageDirectory().getPath()+"/seetaface"+File.separator+"SeetaPointDetector2.0.pts5.ats");  //特征点
                FACERECOGNIZER = new FaceRecognizer2(Environment.getExternalStorageDirectory().getPath()+"/seetaface"+File.separator+"SeetaFaceRecognizer2.0.ats");  //人脸匹配
